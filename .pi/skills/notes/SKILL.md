...
| **Unclear or ambiguous** | — | **Ask using ask_question before creating anything** |
...
3. **Multiple matches at any priority**: use ask_question to present candidates and let the user pick.
...
If only a first name is given and you can't infer the full name from context, ask using ask_question.
...
4. Results:
   ...
   - **Multiple matches**: use ask_question to let user pick.
...
| Person name ambiguous (multiple matches) | Use ask_question to clarify |
| Request is completely vague (e.g., "create a note") | Use ask_question to clarify category before doing anything |
