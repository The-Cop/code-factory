...
Parse the user's invocation prompt for reference deck paths (`.pptx`) and any brand or output description. Resolve the brand DNA like this:

- If one or more `.pptx` paths are supplied in the user's invocation prompt, run Steps 1 and 2 to extract fresh DNA (colors, fonts, backgrounds, word budget) from those decks.
...
| Error | Action |
|-|-|
| No reference decks supplied in the user's invocation prompt | Not an error. Use the built-in Datadog brand DNA and skip to Step 3. |
