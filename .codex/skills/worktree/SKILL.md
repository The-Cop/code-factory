...
Parse the user's invocation prompt for flags:
...
> **Remove which?** -- Which worktree do you want to remove?\n<list of worktree paths with their current branch>
...
Build the worktree slug from the user's invocation prompt (the full remaining text after stripping flags):
